...
function ddy_dump(...$arg)
{
    foreach($arg as $index => $value) {
        $id = 'ddy-dump-' . ($index);
        $str = is_array($value)? json_encode($value, JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT) : $value;
        echo <<<HTML
...
function ddy_http_post($url, $params, $options=array()) {
    if ($params) {
...
function ddy_mongo_regex($pattern, $options) {
    return new \MongoDB\BSON\Regex($pattern, $options);
}

function ddy_echo(...$arg) {
    echo '<pre>';
