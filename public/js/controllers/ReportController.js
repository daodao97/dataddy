...
MetronicApp.controller('ReportController', ['$rootScope', '$scope', '$http', '$log', 'Notification', '$stateParams', '$modal', '$timeout', '$compile', '$interval',
    ...
    window.set_table_options = function(tid, options) {
        ...
        // 将表格搜索框放到表格右上角
        setTimeout(function () {
            var $portlet =  $('div[id="portlet-' + tid + '"]'); 
            var $search =  $portlet.find('div[id="rtable-' + tid + '_filter"] label');
            $search.css({'padding-right': '10px'})
            var $action = $portlet.find('div.actions');
            $action.prepend($search);
        }, 100)
    };
    ...
    $scope.dtOptions = function(tid) {
        ...
        options.responsive = true; // 引入 responsive 插件, 实现响应式, 避免超宽表格表头超出屏幕问题 
...
var render_chart = (function(){
    ...
    function render_amchart(report_id, index, options)
    {
        ...
        if (options.graphs && options.graphs.length > 10) {
            options.legend = {
                enabled: false
            };
        }
    ...
    return function (report_id, options_arr) {
        ...
        for (var i = 0; i < options_arr.length; i++) {
            ...
            if (typeof options == 'string') {
                if (options.startsWith('pie:')) {
                    let token = options.replace("pie:", '').split(',')
                    if (token.length < 1) {
                        $log.error("CHARt[" + report_id + "] pie:category_field,data_field foramt error")
                    }
                    options = {
                        type: 'pie',
                        graphs: [
                            {
                                valueField: token[0] // category field
                            },
                            {
                                valueField: token[1] // data field
                            }
                        ]
                    }
                } else {
            ...
            if (columns.startsWith("line:")) {
                columns = columns.replace("line:", "")
                console.log("columns line:", columns)
            }
